...
export type Expense = {
  ...
  paymentMethod: PaymentMethod;
  note: string | null;
  // Set when this row was generated by a vendor bill rather than entered by
  // hand. Such rows are kept in step by a trigger and are read-only at the
  // database level — edit the bill instead.
...
export type NewExpenseInput = Omit<Expense, 'id' | 'shopId' | 'vendorName' | 'createdBy' | 'createdAt' | 'updatedAt' | 'invoiceId'>;
