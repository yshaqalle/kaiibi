...
//
// `about`/`signup` get an explicit `headerLeft` that always navigates back
// to `login`, rather than relying on the native back button's default
...
// handler removes the ambiguity instead of depending on that. It uses
// `router.back()` rather than `router.replace('/login')`: `about`/`signup`
// are only ever reached by pushing from `login`, so `back()` reliably pops
// to that existing entry instead of pushing a duplicate `login` on top.
