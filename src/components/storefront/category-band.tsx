...
// on it. The scrim is `TILE_SCRIM` (scale.ts) -- a tile-sized twin of the
// shop card's own hero scrim (`storefront-hero-scrim`, theme-shared.tsx,
// `HERO_SCRIM`), not the identical constant: a small tile needs less scrim
// than a full-bleed hero, or the darkening muddies the photograph the tile
// exists to show. See scale.ts's own comment on the two for why they are
// deliberately two named values rather than one shared between them.
  ...
  return (
    ...
    >
      ...
      <LinearGradient
        ...
        colors={TILE_SCRIM.colors}
        locations={TILE_SCRIM.locations}
...
// THE FALLBACK: the pill this band rendered before the sliding indicator
// briefly existed, unchanged -- including its own SELECTED fill, restored
// here now that nothing slides in behind it to supply one.
