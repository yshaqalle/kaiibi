...
export default function StoreScreen() {
  ...
  if (showSetup) return <ShopSetup onClose={() => setShowSetup(false)} />;
  ...
  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Pressable onPress={() => setShowSetup(true)} style={styles.setupLink}><Text style={styles.setupLinkText}>Setting up another shop? Start a shop profile  →</Text></Pressable>
...
function ShopSetup({ onClose }: { onClose: () => void }) {
  ...
  const toggleSelling = (item: string) => setSelling((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item]);
  const next = () => setStep((current) => Math.min(current + 1, 3));

  if (published) return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.setupContent}><Text style={styles.progress}>SHOP SETUP COMPLETE</Text><View style={styles.successCard}><Text style={styles.successIcon}>✓</Text><Text style={styles.successTitle}>{shopName || 'Your shop'} is ready to share.</Text><Text style={styles.successText}>Your storefront is now prepared for customers in {location || 'Hargeisa'}. Add your first products next to make the shop feel complete.</Text><View style={styles.shareBox}><Text style={styles.shareLabel}>YOUR SHOP LINK</Text><Text style={styles.shareUrl}>kaiibi.so/shop/{(shopName || 'your-shop').toLowerCase().replaceAll(' ', '-')}</Text></View><Pressable onPress={onClose} style={styles.save}><Text style={styles.saveText}>Go to inventory</Text></Pressable></View></ScrollView></SafeAreaView>;

  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.setupContent} showsVerticalScrollIndicator={false}>
    <Pressable onPress={onClose}><Text style={styles.back}>← Back to My Shop</Text></Pressable>
    <Text style={styles.progress}>STEP {step} OF 3</Text><Text style={styles.setupTitle}>{step === 1 ? 'Tell customers about your shop.' : step === 2 ? 'Where can customers find you?' : 'What does your shop sell?'}</Text>
    <Text style={styles.setupIntro}>{step === 1 ? 'Start with the basics. You can edit everything later.' : step === 2 ? 'Accurate location and contact information helps local customers trust your shop.' : 'Choose the product groups you plan to list first.'}</Text>
    <View style={styles.stepDots}>{[1, 2, 3].map((number) => <View key={number} style={[styles.stepDot, number <= step && styles.stepDotActive]} />)}</View>
    <View style={styles.setupForm}>
      {step === 1 && <><Text style={styles.fieldLabel}>SHOP NAME</Text><TextInput value={shopName} onChangeText={setShopName} placeholder="e.g. Hilaac Home Goods" placeholderTextColor="#89928B" style={styles.input}/><Text style={styles.fieldLabel}>SHORT DESCRIPTION</Text><TextInput value={description} onChangeText={setDescription} placeholder="What makes your shop special?" placeholderTextColor="#89928B" style={[styles.input, styles.descriptionInput]} multiline textAlignVertical="top"/></>}
      {step === 2 && <><Text style={styles.fieldLabel}>CITY</Text><TextInput value={location} onChangeText={setLocation} placeholder="Hargeisa" placeholderTextColor="#89928B" style={styles.input}/><Text style={styles.fieldLabel}>NEIGHBORHOOD OR LANDMARK</Text><TextInput value={area} onChangeText={setArea} placeholder="e.g. Jigjiga Yar, near the main market" placeholderTextColor="#89928B" style={styles.input}/><Text style={styles.fieldLabel}>CUSTOMER CONTACT</Text><TextInput value={contact} onChangeText={setContact} placeholder="Phone or WhatsApp number" placeholderTextColor="#89928B" keyboardType="phone-pad" style={styles.input}/></>}
      {step === 3 && <><Text style={styles.fieldLabel}>CHOOSE ONE OR MORE CATEGORIES</Text><View style={styles.setupCategories}>{categories.map((item) => <Pressable key={item} onPress={() => toggleSelling(item)} style={[styles.setupCategory, selling.includes(item) && styles.categoryChoiceActive]}><Text style={[styles.categoryChoiceText, selling.includes(item) && styles.categoryChoiceTextActive]}>{selling.includes(item) ? '✓  ' : ''}{item}</Text></Pressable>)}</View><Text style={styles.shareHint}>After publishing, you will get a shareable shop link and can start adding product photos, prices, descriptions, and stock.</Text></>}
      <Pressable onPress={step === 3 ? () => setPublished(true) : next} style={styles.save}><Text style={styles.saveText}>{step === 3 ? 'Create and share my shop' : 'Continue'}</Text></Pressable>
    </View>
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#FAF9F5' }, content: { padding: 20, paddingBottom: 42 }, setupContent: { width: '100%', maxWidth: 650, alignSelf: 'center', padding: 20, paddingBottom: 55 }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }, kicker: { color: '#E45B37', fontWeight: '800', fontSize: 10, letterSpacing: 1.4 }, title: { color: '#17261F', fontSize: 31, letterSpacing: -1.5, fontWeight: '800', marginTop: 4 }, avatar: { width: 43, height: 43, borderRadius: 22, backgroundColor: '#17261F', alignItems: 'center', justifyContent: 'center' }, avatarText: { color: '#fff', fontWeight: '800', fontSize: 12 }, storeStatus: { marginTop: 25, backgroundColor: '#E5EFE3', padding: 13, borderRadius: 12, flexDirection: 'row', alignItems: 'center' }, statusDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#5A9569', marginRight: 8 }, statusText: { fontSize: 13, color: '#31533A', fontWeight: '700', flex: 1 }, statusArrow: { fontSize: 25, color: '#31533A', lineHeight: 20 }, setupLink: { alignSelf: 'flex-start', marginTop: 12 }, setupLinkText: { color: '#47705C', fontSize: 12, fontWeight: '800' }, back: { color: '#47705C', fontSize: 13, fontWeight: '800', marginBottom: 28 }, progress: { color: '#E45B37', fontSize: 10, letterSpacing: 1.4, fontWeight: '800' }, setupTitle: { color: '#17261F', fontSize: 32, lineHeight: 37, letterSpacing: -1.4, fontWeight: '800', marginTop: 8, maxWidth: 500 }, setupIntro: { color: '#5D6C63', fontSize: 14, lineHeight: 21, marginTop: 11, maxWidth: 500 }, stepDots: { flexDirection: 'row', gap: 6, marginTop: 22, marginBottom: 20 }, stepDot: { height: 5, flex: 1, backgroundColor: '#DCE2D9', borderRadius: 3 }, stepDotActive: { backgroundColor: '#E45B37' }, setupForm: { backgroundColor: '#EEF2EB', borderRadius: 16, padding: 16 }, setupCategories: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 19 }, setupCategory: { backgroundColor: '#FFFFFF', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 9, borderWidth: 1, borderColor: '#D9E0D6' }, shareHint: { color: '#637268', fontSize: 12, lineHeight: 18, marginBottom: 17 }, successCard: { backgroundColor: '#E1EEE0', borderRadius: 20, padding: 24, marginTop: 13 }, successIcon: { width: 38, height: 38, borderRadius: 19, overflow: 'hidden', textAlign: 'center', paddingTop: 7, backgroundColor: '#47705C', color: '#FFFFFF', fontSize: 18, fontWeight: '800' }, successTitle: { color: '#17261F', fontSize: 27, lineHeight: 32, letterSpacing: -1.1, fontWeight: '800', marginTop: 17 }, successText: { color: '#4E6655', fontSize: 14, lineHeight: 21, marginTop: 9 }, shareBox: { backgroundColor: '#FFFFFF', borderRadius: 10, padding: 14, marginTop: 20, marginBottom: 17 }, shareLabel: { color: '#718176', fontSize: 9, letterSpacing: 1, fontWeight: '800' }, shareUrl: { color: '#31533A', fontSize: 13, fontWeight: '800', marginTop: 5 }, overview: { fontSize: 18, fontWeight: '800', color: '#17261F', marginTop: 30, marginBottom: 13 }, metricRow: { flexDirection: 'row', gap: 8 }, metric: { flex: 1, minHeight: 103, padding: 13, borderRadius: 14, justifyContent: 'flex-end' }, metricValue: { color: '#17261F', fontSize: 22, letterSpacing: -1, fontWeight: '800' }, metricLabel: { marginTop: 4, color: '#526058', fontSize: 11, lineHeight: 14 }, sectionHeader: { marginTop: 35, marginBottom: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sectionTitle: { color: '#17261F', fontSize: 22, fontWeight: '800', letterSpacing: -.7 }, sectionSub: { color: '#798078', fontSize: 12, marginTop: 3 }, addProduct: { backgroundColor: '#17261F', borderRadius: 10, paddingHorizontal: 13, paddingVertical: 10 }, addProductText: { color: '#fff', fontWeight: '800', fontSize: 12 }, form: { backgroundColor: '#EEF2EB', borderRadius: 15, padding: 15, marginBottom: 14 }, formTitle: { fontSize: 17, fontWeight: '800', color: '#17261F', marginBottom: 15 }, fieldLabel: { fontSize: 10, letterSpacing: 1, fontWeight: '800', color: '#657269', marginBottom: 7, marginTop: 3 }, photoPicker: { height: 146, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#D5DED2', borderStyle: 'dashed', borderRadius: 11, marginBottom: 12, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }, photoPreview: { width: '100%', height: '100%' }, photoIcon: { color: '#3F7850', fontSize: 27, lineHeight: 29 }, photoTitle: { color: '#31533A', fontSize: 13, fontWeight: '800', marginTop: 4 }, photoHint: { color: '#78867C', fontSize: 11, marginTop: 2 }, input: { backgroundColor: '#fff', borderRadius: 9, paddingHorizontal: 11, height: 43, color: '#17261F', marginBottom: 8 }, descriptionInput: { height: 78, paddingTop: 11 }, formRow: { flexDirection: 'row', gap: 8 }, halfInput: { flex: 1 }, categoryChoices: { gap: 7, paddingBottom: 12 }, categoryChoice: { backgroundColor: '#FFFFFF', paddingVertical: 8, paddingHorizontal: 11, borderRadius: 16, borderWidth: 1, borderColor: '#D9E0D6' }, categoryChoiceActive: { backgroundColor: '#31533A', borderColor: '#31533A' }, categoryChoiceText: { fontSize: 11, fontWeight: '700', color: '#546158' }, categoryChoiceTextActive: { color: '#FFFFFF' }, tagHelp: { color: '#78867C', fontSize: 11, marginTop: -2, marginBottom: 12 }, save: { backgroundColor: '#E45B37', height: 42, borderRadius: 9, justifyContent: 'center', alignItems: 'center', marginTop: 3 }, saveText: { color: '#fff', fontWeight: '800' }, inventory: { backgroundColor: '#fff', borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: '#EFEEE9' }, item: { minHeight: 76, flexDirection: 'row', alignItems: 'center', padding: 11, borderBottomWidth: 1, borderBottomColor: '#F0EFEB' }, thumb: { width: 47, height: 53, borderRadius: 10, backgroundColor: '#E8DCCB', alignItems: 'center', justifyContent: 'center' }, itemImage: { width: 47, height: 53, borderRadius: 10, backgroundColor: '#E8DCCB' }, thumbText: { fontFamily: 'serif', fontSize: 24, color: '#6B5B48' }, itemInfo: { flex: 1, marginLeft: 11 }, itemName: { color: '#26342D', fontSize: 13, fontWeight: '700' }, itemMeta: { color: '#8A908A', fontSize: 10, marginTop: 3 }, itemPrice: { color: '#7B837C', fontSize: 12, marginTop: 3 }, stockInfo: { alignItems: 'flex-end' }, stockStatus: { color: '#438254', fontSize: 10, fontWeight: '800' }, lowStock: { color: '#D27631' }, stockCount: { color: '#7B837C', fontSize: 11, marginTop: 4 }, tip: { flexDirection: 'row', gap: 12, backgroundColor: '#F3E9D8', borderRadius: 15, marginTop: 22, padding: 17, alignItems: 'flex-start' }, tipSpark: { color: '#E45B37', fontSize: 20 }, tipTitle: { color: '#5D4930', fontWeight: '800', fontSize: 14 }, tipText: { color: '#7A6549', fontSize: 12, marginTop: 3 },
});
