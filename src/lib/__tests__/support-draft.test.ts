...
describe.each(['web', 'ios'] as const)('support draft on %s', (os) => {
  ...
  async function store(raw: string) {
    if (os === 'web') {
  ...
  it('round-trips a draft', async () => {
    ...
    expect(await readStoredDraft()).toEqual(draft);
  });
  ...
  it('refuses a category that no longer exists', async () => {
    await store(JSON.stringify({ ...draft, category: 'printers' }));

    expect(await readStoredDraft()).toBeNull();
  });

  it('keeps a draft whose category is still null', async () => {
    await store(JSON.stringify({ ...draft, category: null, area: null }));

    expect(await readStoredDraft()).toEqual({ ...draft, category: null, area: null });
  });

  it('survives corrupt JSON', async () => {
    await store('{"category":"broken",');
  ...
  it('replaces fields of the wrong type with empty ones', async () => {
    await store(JSON.stringify({ category: 'billing', area: 7, subject: null, details: ['a'] }));

    expect(await readStoredDraft()).toEqual({
      category: 'billing',
      area: null,
      areaOther: '',
      subject: '',
  ...
  it('falls back to in-app for an unknown contact preference', async () => {
    await store(JSON.stringify({ ...draft, contactPreference: 'sms' }));

    expect((await readStoredDraft())?.contactPreference).toBe('in_app');
  });
});
