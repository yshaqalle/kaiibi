-- "Sales"/promotions the owner configures once in Settings and which then
-- auto-apply in the POS cart to any matching product — store-wide, or
-- scoped to one brand or one category. `scope_value` holds the brand or
-- category name for those two scopes, and is null for 'store'.
create table public.promotions (
  ...
  scope text not null check (scope in ('store', 'brand', 'category')),
  scope_value text,
  active boolean not null default true,
  created_at timestamptz not null default now()
);
create index promotions_shop_id_idx on public.promotions(shop_id);
...
-- Per-line discount (from an auto-applied promotion, or a manual cashier
-- override in the POS cart — the client resolves which one applies and
-- sends the resulting cents figure either way) and a transaction-level
-- discount entered at checkout, independent of any per-line ones.
alter table public.sale_items add column if not exists discount_cents integer not null default 0;
alter table public.sales add column if not exists discount_cents integer not null default 0;

drop function if exists public.complete_sale(uuid, jsonb, jsonb, text, text, text, text);
...
grant execute on function public.complete_sale(uuid, jsonb, jsonb, text, text, text, text, integer) to authenticated;

-- edit_sale gets the same discount fields, so editing a sale (e.g. changing
-- a quantity) doesn't silently wipe out discounts that were applied at
-- checkout — the app re-sends each item's original discount_cents
-- unchanged unless the cashier explicitly changes it.
drop function if exists public.edit_sale(uuid, jsonb, jsonb, text, text, text);
...
grant execute on function public.edit_sale(uuid, jsonb, jsonb, text, text, text, integer) to authenticated;
