-- A sale's entry is dated in the shop's local time, and a sale whose month has
-- already closed posts to the open one.
--
-- complete_sale is reproduced VERBATIM from
-- supabase/migrations/20260908000200_post_complete_sale.sql -- which is its
-- newest CREATE OR REPLACE definition and, since 20260908000200 baked the
-- lock-order ORDER BY in, also its newest LIVE state. edit_sale and
-- settle_sale_balance are NOT redefined here, so 20260831000100 and
-- 20260831000200 remain their newest definitions.
--
-- Both fixes are in ONE migration on purpose. This function has been copied
-- forward nineteen times and the repo has already lost an edit that way
-- (the loyalty maturation guard, unenforced for four migrations); two
-- migrations here would mean two more ~400-line reproductions and two more
-- chances to drop something.
--
-- Three declarations and two edits, and nothing else differs from
-- 20260908000200:
--
--   1. v_entry_date, v_period_status, v_posted_date in the DECLARE section.
--
--   2. THE ENTRY DATE IS THE SHOP'S LOCAL DATE.
--      It was `coalesce(p_created_at, now())::date`, which resolves in the
--      database session's timezone -- UTC on Supabase. Somalia is UTC+3, so a
--      sale rung up at 01:30 local on the 1st is 22:30 UTC on the last day of
--      the PREVIOUS month, and posted to the wrong period. src/lib/period.ts
--      buckets the sales report in device-local time, so the ledger and the
--      sales report disagreed for every late-night sale at a month boundary --
--      permanently, because once that period closes a posted entry cannot be
--      re-dated.
--
--      'Africa/Mogadishu' is a PLATFORM CONSTANT, deliberately. Every market
--      kaiibi serves is UTC+3 -- Somalia, Somaliland, Ethiopia, Djibouti,
--      Kenya -- so one constant is correct today for every shop on the system.
--      There is deliberately NO shops.timezone column: adding one means a
--      migration, a settings screen, a default for every existing shop, and a
--      second source of truth for src/lib/period.ts to learn about, which is a
--      bigger change than this problem justifies right now. It was considered
--      and declined, not missed. When kaiibi sells into a market that is not
--      UTC+3, this expression is what becomes a per-shop setting, and it is the
--      only place in this function that has to change.
--
--   3. A SALE DATED INTO A CLOSED PERIOD POSTS TO THE CURRENT ONE.
--      open_period_for raises for any non-open period, and
--      src/lib/sales-import.ts:126 passes p_created_at for every CSV-imported
--      historical sale -- so once a shop closed any month, importing sales into
--      it failed the whole row group with a ledger error on an import screen.
--
--      Redating is the correct accounting treatment, not a workaround: a
--      transaction that arrives after its month has closed posts to the open
--      period. That is what closing MEANS. The sale row keeps its true date in
--      sales.created_at; only the recognition moves, and the entry's own
--      description says so, so a reader of the journal can see why an August
--      sale is sitting in October without going back to the source row.
--
--      Implemented by READING the period's status before calling
--      post_journal_entry rather than by catching open_period_for's exception:
--      an exception handler around the post would also swallow a genuinely
--      broken chart of accounts, an unbalanced entry, or a missing account, and
--      retry them into the current period as if nothing were wrong.
--
--      If the CURRENT period is itself closed or locked, post_journal_entry
--      still raises and the sale still fails. That is correct and deliberate:
--      a shop with no open period at all is a genuinely broken state, and there
--      is no fallback that is not a lie about where the money went.
--
-- The signature is unchanged, so `create or replace` genuinely replaces rather
-- than adding an overload -- no drop is needed here.
--
-- CARRIED FORWARD BY HAND, as it had to be in 20260908000200: the ORDER BY on
-- the item loop, from 20260905000000_complete_sale_lock_order. That migration
-- patches complete_sale by TEXT SUBSTITUTION against the live pg_proc source
-- instead of re-creating it, so it is invisible both to a grep for this
-- function's CREATE OR REPLACE header and to accumulated-rpc-edits.test.ts,
-- which reads migration text. It is guarded by a COMPLETE_SALE_EDITS entry now
-- that it lives in a real definition. See the comment at the loop itself.
