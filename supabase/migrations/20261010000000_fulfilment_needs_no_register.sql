...
-- named above would silently revert a shipped fix -- the failure
-- accumulated-rpc-edits.test.ts exists for, and the one that left the loyalty
-- maturation guard unenforced for four migrations.
--
...
--
-- 20260929000250 landed AFTER 20260929000200 and re-created
-- complete_storefront_order in full to give an out-of-range order line a typed
-- `order_line_out_of_range` refusal instead of complete_sale's raw English. It
-- did not touch complete_sale, which is why complete_sale's newest definition
-- is still the older file's. Copying complete_storefront_order from
-- 20260929000200 -- as this task's brief said to -- would have dropped that
