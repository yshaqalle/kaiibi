-- The open-register guard is not a client-side option, on the database that
-- already shipped one.
--
-- ── WHY THIS FILE EXISTS AT ALL, WHEN main ALREADY HAS THE RIGHT CODE ───
-- Two branches implemented "fulfilment needs no till" independently and both
-- called their migration 20261010000000_fulfilment_needs_no_register.sql. The
-- other one reached production first. It gave complete_sale a sixteenth
-- argument, `p_require_register boolean default true`, and had
-- complete_storefront_order pass false. main's version -- same filename,
-- different body -- has no such argument and reads the exemption off a
-- provenance row instead.
--
-- `supabase db push` keys a migration by its VERSION and nothing else, and
-- production's supabase_migrations.schema_migrations already records
-- 20261010000000 as applied. main's file will therefore NEVER run there, no
-- matter how right it is. The only thing that reaches that database is a
-- migration with a version it has not seen. This is that migration, and its
-- entire job is to make production's two functions identical to main's.
--
-- ── THE DEFECT IT IS CORRECTING ─────────────────────────────────────────
-- complete_sale is granted to `authenticated` and exposed over PostgREST, so
-- every parameter it declares is a field any member holding `pos.access` can
-- put in a JSON body. A default decides what happens when the client says
-- nothing; it decides nothing at all about what happens when the client
-- speaks. The guard four screens down says so in its own comment -- "the
-- client is the party it is meant to constrain" -- and a parameter named in
-- that guard's own condition handed that same client the off switch.
--
-- Reproduced by applying production's exact file over a `db reset` database
-- and calling as role `authenticated` with a real JWT claim, at a shop whose
-- primary location has require_open_register set:
--
--   default                     => REFUSED
...
--
-- Both were one extra JSON field. `false` is the obvious one; `null` is the
-- sharper one, because it defeats the guard without ever looking like a
-- decision to defeat it.
--
-- ── WHY A DROP, AND WHY `if exists` ─────────────────────────────────────
-- `create or replace function` cannot change an argument list, so the
-- sixteen-argument function has to go before the fifteen-argument one can
-- come back; leaving both would make a call naming only the first three
-- parameters ambiguous. The drop names the sixteen-argument signature exactly.
--
-- It is `drop function IF EXISTS` because this file has to be correct on TWO
-- DIFFERENT DATABASES:
--
--   * on production, the sixteen-argument function is really there and the
--     drop is the whole point;
--   * on a database built from this repo's migrations -- `db reset`, and
--     every CI run -- the sixteen-argument version NEVER EXISTED, because the
--     20261010000000 in this tree is main's. An unconditional drop would
--     fail there and take the migration chain down with it.
--
-- So the drop is a no-op locally and corrective on production, and both
-- databases land on the same fifteen-argument function either way.
--
-- ── complete_storefront_order IS NOT OPTIONAL HERE ──────────────────────
-- Production's complete_storefront_order calls
-- `complete_sale(... p_require_register => false)`. The moment the
-- sixteen-argument function is dropped that call resolves to no function at
-- all and storefront fulfilment breaks entirely -- every handover, at every
-- shop, not only the ones requiring a register. It is re-created below as
-- main's version, which opts out via the storefront_order_fulfilments
-- provenance row instead of an argument, so dropping and recreating both
-- functions in one migration is the only ordering that is never broken
-- between statements.
--
-- ── WHERE THE TWO BODIES BELOW WERE COPIED FROM ─────────────────────────
-- Re-derived per function rather than assumed, the way 20261010000000's own
-- header insists on, because two functions edited in one migration can have
-- different newest ancestors and 20260929000250 is the standing proof:
--
--   $ grep -l 'create or replace function public.<name>(' \
--       supabase/migrations/*.sql | sort | tail -1
--
--   complete_sale             <- 20261010000000_fulfilment_needs_no_register.sql:137
--   complete_storefront_order <- 20261010000000_fulfilment_needs_no_register.sql:1256
--
-- This time they ARE the same file: 20261010000000 re-created both, and the
-- only migration after it, 20261010000100, touches get_public_storefront and
-- nothing else. Both bodies below are that file's, reproduced WHOLE and
-- unaltered -- not a diff against it -- so the next reader never has to replay
-- a chain of substitutions to know what these functions do.
--
-- ── WHAT ELSE DIFFERS BETWEEN THE TWO 20261010000000s: NOTHING ──────────
-- The two files were written independently, so "the parameter" was checked
-- rather than assumed. Comment-stripped, the production file and main's
-- differ in exactly five places, all one mechanism: the drop line, the
-- parameter declaration, the guard's `if` condition, the call site in
-- complete_storefront_order, and the signature spelled out in the two grant
-- statements. Every other statement in 1585 lines is byte-identical. There is
-- no second divergence being papered over here.
--
-- ── THE GRANTS, AND WHY THE REVOKE IS LOAD-BEARING THIS TIME ────────────
-- Postgres grants EXECUTE to PUBLIC on every newly created function, and an
-- ACL does not survive a drop. On production the fifteen-argument function
-- below is genuinely NEW -- there is no ACL to inherit -- so `grant execute
-- to authenticated` on its own would leave the function callable by PUBLIC,
-- which is a strictly WIDER grant than the sixteen-argument one it replaces.
-- The revoke is what makes that not happen, and it must come first. On a
-- fresh local database the same two statements merely restate what
-- 20261010000000 already said. Written unconditionally so that a reader does
-- not have to know which database they are looking at to know who can call
-- these.

drop function if exists public.complete_sale(uuid, jsonb, jsonb, text, text, text, text, integer, uuid, timestamptz, uuid, integer, uuid, boolean, boolean, boolean);

-- ── complete_sale, re-created in full at fifteen arguments ──────────────
-- Reproduced verbatim from 20261010000000:137. `create or replace` and not
-- `create`: on production the drop above left nothing behind and this creates
-- it, while on a database built from this repo's migrations the
-- fifteen-argument function is already there and this replaces it with an
-- identical body. One statement, correct on both.
...
declare
  ...
  -- The location this sale will land at, and the completing member's open
  -- session there. Both exist only so a session can be looked up: neither is
  -- passed to complete_sale as p_location_id. See the header.
...
-- ── The grants ──────────────────────────────────────────────────────────
--
-- The revoke goes first, and here it repairs rather than restates: the drop
-- at the head of this file took the sixteen-argument function's ACL with it,
-- so on production complete_sale below is a brand new function carrying
-- Postgres's default EXECUTE to PUBLIC until this line removes it. A bare
-- grant would have been a no-op dressed as a decision. See the header.
...
-- unchanged on both databases -- so its ACL survives and these two restate it,
-- the same way 20260929000200 and 20261010000000 both restate it.
