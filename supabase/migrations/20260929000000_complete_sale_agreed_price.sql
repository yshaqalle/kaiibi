-- A line may be filed at the price the customer was AGREED, not the price on
-- the shelf today.
--
-- ── Why ────────────────────────────────────────────────────────────────────
--
-- public.complete_sale prices every line from products.price_cents. That is
-- the right answer at a till: the correct price of a thing you are selling
-- right now is its price right now, and a cashier's client has no standing to
-- say otherwise.
--
-- A storefront order is a different object. It is a PROMISE MADE EARLIER --
-- order_items froze product_name, unit_price_cents and quantity at checkout
-- (20260926000050) because that is what the customer agreed to -- and when the
-- shop later completes it, the customer must be charged what they were quoted.
-- Today complete_storefront_order (20260928000200) hands the order's lines to
-- complete_sale, complete_sale re-prices them from the shelf, and if the shop
-- has moved a price since, the payment no longer matches the total and the
-- whole completion is refused as `order_total_changed`.
--
-- This migration gives complete_sale the ability to file a line at an agreed
-- price. It does not change any caller: a cart that sends no agreed price is
-- byte-identical to what this function did before, which is what
-- supabase/tests/verify-complete-sale-baseline.sql's thirteen original checks
-- exist to prove.
--
-- ── The field is NEW, and it had to be ─────────────────────────────────────
--
-- Cart payloads have carried a `unit_price_cents` since 0001 and complete_sale
-- has deliberately IGNORED it that whole time. Making that field authoritative
-- would have been the smaller-looking change and it is the wrong one: it
-- silently rewrites what every existing caller charges, on the write path every
-- counter sale in every shop goes through. The baseline pins this on purpose --
-- every cart in it sends `unit_price_cents` 9999, a price no product has, while
-- asserting the product's own price -- so that reading the cart would move
-- every total by thousands and be refused by the function's own
-- payments-equality check.
--
-- So the agreed price is `agreed_unit_price_cents`, a distinct name for a
-- distinct claim, and `unit_price_cents` goes on being ignored. Checks 14 and
-- 19 of the baseline send BOTH fields on the same line for exactly that reason.
--
-- ── What it does, and what it deliberately does not ────────────────────────
--
--   * ABSENT -> the line prices from products.price_cents, exactly as before.
--     `coalesce(v_agreed_price, v_product.price_cents)`, never
--     `case when v_agreed_price > 0` -- which reads naturally and charges list
--     for the item a shop promised to throw in free.
--   * PRESENT -> sale_items.unit_price_cents and line_total_cents follow it,
--     and so does the revenue credit. The gap between the agreed price and the
--     shelf price does NOT reach 4200: a discount is money the shop gave away
--     off the price of this sale, and here the agreed price IS the price of
--     this sale. There is nothing to contra.
--   * unit_cost_cents STILL COMES FROM THE PRODUCT'S CURRENT COST. Cost is what
--     the shop actually paid; an agreement about the selling price says nothing
--     about it. Moving it would misstate COGS and every gross-profit figure
--     downstream of it, and would make giving stock away look free.
--   * AN AGREED PRICE PLUS A PROMOTION ON THE SAME LINE RAISES. See the guard
--     itself for the argument. Named in stable text so a client can match the
--     prefix and say it in the shopkeeper's own words.
--   * A NEGATIVE OR ABSURD AGREED PRICE RAISES. It is input from a caller, not
--     truth. The ceiling is on the LINE and is measured in bigint, so a line
--     that would overflow a 32-bit integer is a sentence rather than
--     `integer out of range` from the middle of the register's write path.
--
-- TAX IS NOT THIS MIGRATION'S PROBLEM. An order quoted tax-exclusive still has
-- the shop's tax added on top here, which is the other half of why
-- `order_total_changed` fires. That is the next task's.
--
-- NOT GATED ON discounts.manual, deliberately. That permission exists to stop a
-- cashier typing an arbitrary number off a price at the counter, and an agreed
-- price is not that -- it is the shop's own earlier quote being honoured, and
-- the member completing a storefront order is not the party who set it. Gating
-- it would defeat the entire purpose of this change for the shops least likely
-- to hand out the discounts.manual permission. It is worth saying out loud that
-- this DOES widen what a caller holding pos.access can do: a client that lies
-- about an agreed price can undercut the shelf price without discounts.manual.
-- The bounds above stop it being unbounded, not being possible.
--
-- ── Copied forward ─────────────────────────────────────────────────────────
--
-- complete_sale is re-created IN FULL, from
-- 20260908000300_sale_entry_date.sql -- its NEWEST definition, not an older
-- ancestor. This repo has already lost an edit to a copy-forward from the wrong
-- ancestor (the loyalty maturation guard, unenforced for four migrations), and
-- supabase/tests/accumulated-rpc-edits.test.ts exists to make that impossible
-- to do quietly. Four edits and three declarations differ from 20260908000300
-- and NOTHING else does:
--
--   1. c_max_line_cents, v_unit_price and v_agreed_price in the DECLARE block.
--   2. The agreed-price block in the item loop: parse, refuse a promotion,
--      refuse a negative, refuse an out-of-range line, resolve v_unit_price.
--   3. `v_line := v_unit_price * v_qty - v_line_discount` (was
--      v_product.price_cents).
--   4. The sale_items insert files v_unit_price (was v_product.price_cents).
--      unit_cost_cents is untouched.
--
-- Entries for all four are added to accumulated-rpc-edits.test.ts, as its
-- header instructs, so the next copy-forward cannot lose them either.
--
-- The signature is unchanged, so `create or replace` genuinely replaces rather
-- than adding an overload -- no drop is needed, and the agreed price rides in
-- the p_items payload where it belongs: it is a property of a LINE, not of the
-- sale.
...
begin
  ...
  loop
    ...
    v_agreed_price := nullif(v_item->>'agreed_unit_price_cents', '')::integer;
