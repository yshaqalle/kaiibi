-- A settlement posts at the till that took the money, and a refund goes back
-- out of the tenders it came in on.
--
-- Both functions are reproduced IN FULL from
-- 20260908000350_post_refund_and_settlement.sql -- which itself copied them
-- forward from 20260831000200_refund_goods_not_cash.sql with a posting side --
-- with exactly two changes, both on the posting side, both described below.
-- Nothing else differs; `diff` the two files and only those blocks move.
-- Verified again against 20260905000000_complete_sale_lock_order.sql, which
-- patches functions by substituting pg_proc.prosrc at runtime and therefore
-- leaves its fix in no migration text at all: that migration names only
-- complete_sale and edit_sale, so neither function below carries a live fix
-- that copying forward would silently revert.
--
-- ## Change 1: a settlement's entry belongs to the till that took the cash
--
-- 20260908000350 stamped the settlement entry with `v_sale.location_id` -- the
-- branch that RANG the sale, which is not necessarily the branch the money was
-- handed over at days later.
--
-- That re-opened, on the ledger side, exactly the bug
-- 20260831000300_settlement_cash_belongs_to_its_till.sql exists to close on the
-- drawer side. That migration made register_session_expected attribute a
-- payment through `coalesce(sp.register_session_id, s.register_session_id)`,
-- because "the money is handed over days later at whatever till is open" and
-- the till that took the cash was closing with a surplus variance it could not
-- explain. Leaving the JOURNAL on the sale's location means a customer settling
-- a Branch-A sale at Branch B credits Branch B's till in
-- register_session_expected and debits Branch A's 1000 Cash in the ledger: two
-- views of the same cash that disagree permanently, and a per-branch P&L that
-- is wrong in both branches at once.
--
-- So: `coalesce(v_session.location_id, v_sale.location_id)`, the ledger's
-- spelling of that migration's coalesce. register_sessions.location_id is NOT
-- NULL, so the fallback fires only when no session was passed at all -- a
-- settlement taken outside a till session, which is the only case where the
-- sale's own branch is the best answer available.
--
-- ## Change 2: a refund on a split-tender sale goes back out of every tender
--
-- 20260908000350 credited the WHOLE refunded cash amount to the single largest
-- payment method. complete_sale posts one debit line per payment method, and
-- verify-posting-sales check 2 pins that: "two lines against two accounts is
-- the whole reason the drawer and the wallet can be reconciled separately".
-- The refund has to mirror that shape or the two sides stop reconciling.
--
-- register_session_expected already pro-rates a refund across tenders
-- (20260831000300:44-59): per cash bucket, `r.total_cents * sp.amount_cents /
-- s.total_cents`. A sale of 600 paid 100 cash + 500 zaad and later refunded
-- 200 therefore takes 33 out of the CASH drawer -- while the old lump credited
-- 1020 Zaad with the whole 200 and 1000 Cash with nothing. The drawer count and
-- the ledger disagreed by 33 with nothing anywhere to explain why.
--
-- So the credit side now emits one line per method, weighted by that method's
-- share of what was collected -- the same weights register_session_expected
-- uses. The denominator is what was COLLECTED rather than sales.total_cents,
-- and on the fully-paid sale that both formulas were written for those are the
-- same number. They differ only on a part-paid sale, where the ledger's lines
-- must still sum exactly to the cash going out or the entry does not balance
-- (see the receivable line below); register_session_expected's own denominator
-- is the sale total there, which is a drawer-side question and not this
-- migration's to answer.
--
-- Rounding: LARGEST REMAINDER. Every method gets floor(share), and the cents
-- left over go one each to the largest fractional parts. Chosen over "give the
-- whole difference to the biggest method" because every line then lands within
-- a cent of its exact share and none can come out NEGATIVE -- a negative credit
-- is a debit, i.e. a refund that puts money INTO a tender. The lines sum to the
-- cash refunded exactly, by construction. A method whose share rounds to zero
-- is omitted rather than posted: journal_lines carries check (amount_cents <>
-- 0), so a zero line fails the whole refund.
--
...
--                              one line per tender it came in on
...
declare
  ...
  -- The credit lines for the cash going back, one per tender it came in on.
  -- Built from sale_payments, not from sales.payment_method -- see below.
  v_method_lines jsonb;
...
begin
  ...
  if v_total_cents > 0 then
    -- ONE LINE PER TENDER, pro-rated -- not a lump against the biggest method.
    --
    -- complete_sale posts one debit line per payment method and
    -- verify-posting-sales check 2 pins that, because two lines against two
    -- accounts is the whole reason the drawer and the wallet reconcile
    -- separately. A refund that collapsed back to one account would undo that
    -- on the way out.
    --
    -- The weights are register_session_expected's own (20260831000300:44-59:
    -- `r.total_cents * sp.amount_cents / s.total_cents` per cash bucket), so
    -- the drawer and the ledger cannot drift: a 600 sale paid 100 cash + 500
    -- zaad and refunded 200 takes 33 out of the cash drawer THERE and credits
    -- 1000 Cash with 33 HERE. The lump credited 1020 Zaad with all 200 and the
    -- two disagreed by 33 forever.
    --
    -- Read from the sale's PAYMENTS, not from sales.payment_method: that column
    -- reads 'unpaid' until money arrives and account_code_for_payment_method
    -- raises on it, so a refund against a part-paid credit sale would die on
    -- the mapping rather than post. Guaranteed to find at least one row:
    -- v_total_cents > 0 requires v_collected_cents > 0.
    --
    -- Denominator is v_collected_cents, what was actually taken, not
    -- v_sale_total_cents. On a sale paid in full they are the same number, so
    -- this IS register_session_expected's basis for every sale either formula
    -- was written for. They part only on a part-paid sale, and there the
    -- ledger's lines have to sum to v_total_cents EXACTLY -- the 1100 line
    -- below is v_goods_cents - v_total_cents, so a cash side that summed to
    -- anything else would not balance.
    --
    -- LARGEST REMAINDER for the odd cents: floor every share, then hand the
    -- leftover cents out one each, biggest fractional part first. Preferred to
    -- dumping the whole difference on the largest method because every line
    -- then sits within a cent of its exact share and none can go NEGATIVE -- a
    -- negative credit is a debit, a refund that puts money INTO a tender. Ties
    -- break on the bigger tender and then on the method name, so the split is
    -- deterministic and a replay of the same refund produces the same lines.
    with per_method as (
      select sp.method, sum(sp.amount_cents)::numeric as collected
        from public.sale_payments sp
       where sp.sale_id = p_sale_id and sp.amount_cents <> 0
       group by sp.method
    ),
    shares as (
      select method, collected,
             v_total_cents::numeric * collected / v_collected_cents as exact
        from per_method
    ),
    ranked as (
      select method, collected, floor(exact)::integer as base,
             sum(floor(exact)::integer) over () as base_total,
             row_number() over (order by exact - floor(exact) desc, collected desc, method) as rn
        from shares
    )
    select coalesce(jsonb_agg(jsonb_build_object(
             'code', public.account_code_for_payment_method(method),
             'amount_cents', -(base + case when rn <= v_total_cents - base_total then 1 else 0 end),
             'memo', 'Refunded by ' || method)
             order by method), '[]'::jsonb)
      into v_method_lines
      from ranked
     -- Omitted, never posted as zero: journal_lines carries
     -- check (amount_cents <> 0). A tender too small to earn a cent of a small
     -- refund gets no line, and the remaining lines still sum to v_total_cents.
     where base + case when rn <= v_total_cents - base_total then 1 else 0 end > 0;

    v_lines := v_lines || v_method_lines;
  end if;
