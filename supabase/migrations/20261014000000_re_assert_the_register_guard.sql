-- Re-assert the register guard, under a version number production has not seen.
--
-- THIS FILE ADDS NO NEW BEHAVIOUR. It is byte-for-byte the operative half of
-- 20261011000000_the_register_guard_is_not_a_parameter.sql -- the same drop,
-- the same two functions, the same grants -- carrying a NEW VERSION NUMBER so
-- that `supabase db push` will actually run it.
--
--
-- ## Why re-running an applied migration is impossible, and why that matters
--
-- `db push` keys a migration by its VERSION, the leading timestamp in the
-- filename, and it will never re-run a version already listed in
-- supabase_migrations.schema_migrations. It does not compare bodies. It does
-- not compare anything but that list of numbers.
--
-- Production recorded 20261010000000 as applied while holding a DIFFERENT
-- function body than the file of that name in this repo -- a second branch
-- deployed its own version of that timestamp first (scripts/diagnose-
-- production.sql's own header records this). Once a number is on that list,
-- the file that number names becomes unreachable forever.
--
-- So `db push` reporting "up to date" is not evidence that the fix is
-- deployed. It is evidence that a NUMBER is recorded. If 20261011000000 was
-- recorded without its body being what this repo holds, the register hole is
-- still open on production and no amount of pushing will close it.
--
-- A new number is the only lever that reaches a database in that state.
--
--
-- ## What the hole is, restated so this file stands alone
--
-- `complete_sale` briefly carried a sixteenth parameter,
-- `p_require_register boolean default true`. The argument for it was that the
-- default left every existing caller unchanged. That argument was the hole: a
-- function granted to `authenticated` is exposed over PostgREST, so every
-- parameter it declares is a field any caller can send. Reproduced on a live
-- database with a real JWT, `=> false` defeated the shop's
-- require_open_register setting, and `=> null` defeated it too, because
-- `if NULL and ...` is NULL and the guard never fired at all. Both were one
-- extra JSON field.
--
-- The drop below is the fix, and it has to be a DROP: `create or replace
-- function` cannot change an argument list, so the sixteen-argument function
-- must go before the fifteen-argument one can come back. Leaving both would
-- make a call naming only the first three parameters ambiguous -- and, far
-- worse, would leave the off-switch callable.
--
--
-- ## Why this is safe to run on a database that is already correct
--
-- Every statement in it is idempotent:
--
--   * `drop function IF EXISTS` on the sixteen-argument signature is a no-op
--     where that function never existed -- which is every database built from
--     this repo's migrations, including `db reset` and CI.
--   * `create or replace function` on both functions lands the definitions
--     this repo already holds. If they are already those, nothing changes.
--   * the revokes and grants restate what is already true.
--
-- So it repairs an exposed database and leaves a healthy one byte-identical.
-- That is deliberate: the person running this should not have to know which
-- kind they have before running it.
--
--
-- ## How to verify, and why the catalog is the only honest answer
--
--   psql "$PROD_DB_URL" -f scripts/diagnose-production.sql
--
-- Check 2 is the one. `15 | OK` means the hole is closed; `16 | EXPOSED`
-- means it is not. READ pg_proc, NOT the migration list -- the whole reason
-- this file exists is that the migration list said the right thing while the
-- database held the wrong function.
--
-- Run it BEFORE pushing and AFTER. Before tells you whether production was
-- ever exposed, which nobody currently knows; after tells you this worked.
--
--
-- ## A note on push order
--
-- This is version 20261014000000, above two migrations that are in flight and
-- unmerged at the time of writing: 20261012000000 (Orders Part 2) and
-- 20261013000000 (Orders Part 3). It is numbered above them so the
-- cross-branch collision guard (supabase/tests/migration-version-guard.test.ts)
-- stays green, and because this fix should not wait on either of them.
--
-- The consequence, stated so it is not a surprise: if this is pushed first,
-- those two become lower-numbered-but-unapplied, and the CLI will ask for
-- `--include-all` when they are eventually pushed. That is a prompt, not a
-- problem.
