-- Phase 2 of staff accounts & permissions: staff provisioned via the new
-- provision-staff Edge Function get full (ungated) access to their shop's
-- data -- fine-grained per-permission checks are Phase 3. This migration
-- only decides "is this person allowed in this shop at all", via the new
-- is_shop_member() alongside (never replacing) owns_shop().
--
-- owns_shop() stays admin-only and keeps gating shops insert/update/delete
-- and the roles/shop_members tables themselves (see 0017) -- a staff
-- member, even once this ships, still can't manage other staff or roles.
create or replace function public.is_shop_member(p_shop_id uuid) returns boolean
language sql security definer stable set search_path = public as $$
  select public.owns_shop(p_shop_id) or exists (
    select 1 from public.shop_members
    where shop_id = p_shop_id and user_id = auth.uid() and active
  );
$$;
grant execute on function public.is_shop_member(uuid) to authenticated;

-- Lets a staff member's session read the shop itself (name, tax settings,
-- categories, etc.) -- insert/update/delete stay owner_id-based, unchanged.
create policy "members can view their shop" on public.shops for select
  using (is_shop_member(id));

-- Outer table RLS: every "own X" policy from prior migrations swaps its
-- owns_shop(shop_id) check for is_shop_member(shop_id). This is what
-- actually gates direct table reads/writes that don't go through a
-- security-definer RPC (product CRUD, cashier/promotion/currency CRUD,
-- category/tag/brand create, and every plain select).
...
create policy "own sale_items" on public.sale_items for all
  using (exists (select 1 from public.sales s where s.id = sale_id and is_shop_member(s.shop_id)));
...
create policy "own promotions" on public.promotions for all
  using (is_shop_member(shop_id)) with check (is_shop_member(shop_id));
...
-- Product image storage: same swap so a staff member can add/edit products
-- with photos, matching the products table policy above.
drop policy "owners upload their shop's product images" on storage.objects;
...
drop policy "owners delete their shop's product images" on storage.objects;
...
-- Security-definer RPCs bypass table RLS entirely and do their own internal
-- authorization check -- each one below is reproduced at its current
-- (latest-migration) signature and body, with only that internal check
-- swapped from owns_shop to is_shop_member. Nothing else about these
-- functions changes.
