-- A QUOTED TOTAL ALREADY HAS THE TAX IN IT, so the tax is EXTRACTED from it
-- rather than added on top.
--
-- public.complete_sale is a TILL. At a tax-charging shop it prices every line
-- from the shelf, totals them, and adds the tax at the end -- which is right at
-- a counter, where the shelf price is tax-exclusive and the tax appears on the
-- last line of the receipt.
--
-- A STOREFRONT IS NOT A TILL. It quotes the customer a total and the customer
-- accepts that total. When the shop later completes that order, adding tax on
-- top would charge more than was agreed -- so until this migration a
-- tax-charging shop could not complete a storefront order at all: either the
-- customer is billed a figure they never agreed to, or the payment the client
-- collected fails complete_sale's own payments-equality check and the whole
-- fulfilment is refused.
--
-- 20260929000000 and 20260929000050 gave this function a per-line
-- `agreed_unit_price_cents` so a line can be filed at the price the customer
-- was quoted. This is the other half of the same promise: the total the
-- customer was quoted is the total they pay.
--
--     p_prices_include_tax boolean default false
--
-- FALSE IS EVERY EXISTING CALLER, and the default is the whole of the safety
-- argument: absent, this migration adds nothing to the arithmetic any counter
-- sale has ever done. verify-complete-sale-baseline checks 1-13 are byte-for-
-- byte what Task 1 wrote and stay green; check 26b rings up the identical cart
-- with and without the flag and pins 2520 against 2400.
--
-- ── 1. THE TAX IS WHAT ROUNDS, AND THAT IS THE SUBSTANCE OF THIS CHANGE ────
--
-- Extracting tax from a total is NOT the inverse of adding it, and the two
-- natural ways to write the extraction are not the same function:
--
--     ROUND THE TAX     tax = round(gross * rate / (100 + rate))
--     ROUND THE NET     tax = gross - round(gross * 100 / (100 + rate))
--
-- They agree everywhere except where the exact net lands on a half cent, and
-- there they differ by one. 1001 cents at 4% is such a figure -- 1001 * 100 /
-- 104 is exactly 962.5 and 1001 * 4 / 104 is exactly 38.5 -- so rounding the
-- tax gives 39 and rounding the net gives 38, on the same sale.
--
-- THIS FUNCTION ROUNDS THE TAX, and the reason is not a preference. The till
-- already does: `v_tax_cents := round(v_total_cents * v_tax_rate / 100)` rounds
-- the TAX and lets the other side of the sale carry whatever remainder that
-- leaves. Rounding the NET instead would mean the same shop rounds a different
-- quantity depending on which door a sale came in through, and a shop
-- reconciling 2100 Sales Tax Payable against 4000 Sales Revenue would find a
-- cent that appears or vanishes per order with nothing to explain it. A cent an
-- order is not a rounding detail; it is somebody's afternoon, every month.
--
-- Baseline check 27a is 1001 at 4% and asserts 39 -- the figure where the two
-- formulas DISAGREE, chosen deliberately because Task 1's review found the
-- original baseline could not tell round() from floor(): every figure in it
-- divided evenly. 27b is the round trip at a figure where they agree: check 9
-- charges 74 on 2450 to reach a total of 2524, and quoting 2524 tax-inclusive
-- gives 2450 and 74 straight back.
--
-- BOTH SIDES STILL ADD UP TO THE QUOTE. Whichever is rounded, the other is the
-- remainder -- the function never computes the net independently -- so
-- net + tax = the figure the customer was shown, exactly, always.
--
-- ── 2. LOYALTY EARNS ON THE TAX-EXCLUSIVE FIGURE, BECAUSE THE TILL DOES ────
--
-- This was a decision to make, and it was made by reading the register rather
-- than by preference. In this function v_points_earned is computed BEFORE the
-- tax is added to v_total_cents -- it always has been -- so a counter sale has
-- always earned points on the tax-EXCLUSIVE figure. A pre-quoted sale must
-- therefore earn on the quoted total MINUS the tax extracted from it, or the
-- same basket earns a different number of points depending on which door it
-- came in through, and a customer can see that.
--
-- IT FALLS OUT OF WHERE THE EXTRACTION SITS rather than being coded twice. The
-- extraction subtracts the tax from v_total_cents immediately, so from that
-- statement on v_total_cents is the merchandise figure in both directions and
-- the loyalty line is unchanged -- the same line, doing the same thing, to a
-- figure that means the same thing. 2400 quoted inclusive at 5% earns on 2286
-- and gives 23 points; earning on the quote would give 24. Baseline check 28
-- is that point.
--
-- ── 3. REVENUE IS CREDITED NET OF THE TAX INSIDE THE QUOTE ─────────────────
--
-- Not a presentation choice -- without it a tax-charging shop cannot complete a
-- quoted order at all, because the entry does not balance and post_journal_entry
-- refuses it.
--
-- Writing G for v_gross_cents, D for the order discount, R for the redemption,
-- I for the line and promotion discounts and T for the tax:
--
--   * the customer hands over G - D - R (the quote), so the debits are that
--     money plus the contra D + R + I, which is G + I;
--   * the credits are revenue plus T.
--
-- Revenue at G + I therefore leaves the entry heavier on the credit side by
-- exactly T. Revenue at G + I - T balances, and it is also the honest figure:
-- tax collected for the state was never the shop's earnings, which is the very
-- reason the ordinary path credits 4000 with the goods rather than the total.
--
-- THE DISCOUNTS ARE NOT SPLIT, and that is deliberate. D, R and I stay on 4200
-- at the figures the shop actually took off the price it advertised, rather
-- than being decomposed into a net share and a tax share of their own. A
-- discount report exists to say what was given away; what was given away is
-- what the customer saw come off. The tax is extracted once, from the figure
-- the customer actually pays, which is what the shop actually owes.
--
-- ── 4. TAKING THE TAX OUT OF THE PRICE IS TAKING IT OUT OF THE TILL ────────
--
-- The flag moves no price, so it reads at first as needing no permission. It
-- moves what the shop KEEPS: the same two items at a 5% shop collect 2520 and
-- leave 2400 of revenue without it, and collect 2400 and leave 2286 with it.
-- 114 cents of the shop's own takings, decided by a boolean in a JSON payload.
--
-- That is the same money 20260929000050 gated the agreed price's undercut to
-- protect, reached through a different field, and its header's argument applies
-- here word for word: the register has exactly one rule about a cashier
-- reaching into a price, and it is `discounts.manual`. A new field is not a new
-- justification.
--
-- GATED ON THE EFFECT, NOT ON THE FIELD, which is the same shape as that gate
-- and is what keeps requirement 4 below true. The permission is asked only
-- where there is tax to extract:
--
--     v_prices_include_tax and tax enabled and rate > 0
--
-- At a shop with tax disabled, or with no rate set, the flag produces the
-- byte-identical sale it would have produced without it, so nothing is asked.
-- Baseline check 30 is all three halves: refused at a tax-charging shop,
-- accepted at a tax-free one, and the same member's ordinary till sale at the
-- same tax-charging shop untouched.
--
-- WHAT IT COSTS is what it cost there, and it was measured rather than assumed:
-- 20260826000100:865 backfilled `discounts.manual` onto every role that already
-- held `pos.access`, and all three roles default_shop_roles() seeds carry it.
-- The only shops that feel this are shops that deliberately took the permission
-- away from somebody.
--
-- READ THIS BEFORE WRITING TASK 4. Storefront fulfilment sets this flag, so at
-- a tax-charging shop the member completing an order must hold
-- `discounts.manual` -- the same requirement, and the same warning, that
-- 20260929000050's header gives for an order placed before a price rise.
-- Surface it as a permission problem the shop can act on, with the stable
-- message this function raises, rather than discovering it in a shop.
--
-- ── 5. WHAT IS NOT CHANGED ────────────────────────────────────────────────
--
-- A CALL THAT DOES NOT SET THE FLAG BEHAVES EXACTLY AS IT DID BEFORE. That is
-- the claim, in the form that can be falsified, and these are the reasons it
-- holds rather than assertions that it does:
--
--   * the extraction block is wholly inside `if v_prices_include_tax ...`;
--   * v_included_tax_cents is 0 for such a call, so the revenue credit is
--     arithmetically the same expression it was;
--   * the tax line the till has always run is unchanged, and only its guard
--     gains `and not v_prices_include_tax`;
--   * `v_total_cents := v_total_cents + v_tax_cents` is where it was and adds
--     what it always added;
--   * the permission check cannot fire without the flag.
--
-- AT A SHOP WITH TAX DISABLED THE FLAG CHANGES NOTHING -- no extraction, no
-- permission, no 2100 line, no rate snapshotted. Baseline check 29.
--
-- EVERYTHING 20260929000050 BUILT IS PRESERVED: the `discounts.manual` gate on
-- an undercut, the refusal of an agreed price alongside a promotion, and both
-- bounds -- c_max_int_cents on every line and on the running total,
-- c_max_line_cents only where an agreed price was supplied. Baseline checks
-- 14-25 are unchanged and green.
--
-- THE FLAG IS NOT TIED TO THE AGREED PRICE, deliberately. They are two
-- independent statements about a cart -- "this line is filed at the price we
-- quoted" and "the prices in this cart already include tax" -- and a shop whose
-- shelf prices are tax-inclusive is a real shop whether or not it has a
-- storefront. Task 4 will send both together, which baseline check 27b rings up
-- end to end; the sale-level flag with no agreed price anywhere is check 26a.
--
-- ── Copied forward ────────────────────────────────────────────────────────
--
-- complete_sale is re-created IN FULL from 20260929000050 -- its NEWEST
-- definition, which is NOT the 20260908000300 an older plan points at. This
-- repo has already lost an edit to a copy-forward from the wrong ancestor (the
-- loyalty maturation guard, unenforced for four migrations), and
-- supabase/tests/accumulated-rpc-edits.test.ts exists to make that impossible
-- to do quietly. FIVE edits and TWO declaration changes differ from
-- 20260929000050 and nothing else does:
--
--   1. the trailing parameter, and v_prices_include_tax / v_included_tax_cents
--      in the DECLARE block.
--   2. a refusal before the first write: the flag needs discounts.manual where
--      there is tax to extract.
--   3. the extraction itself, which rounds the TAX and takes it back out of the
--      running total.
--   4. `and not v_prices_include_tax` on the add-on branch.
--   5. the revenue credit less v_included_tax_cents.
--
-- Entries for all five are added to accumulated-rpc-edits.test.ts, as its
-- header instructs, so the next copy-forward cannot lose them either. Each
-- token was checked by DELETING the code it guards and watching the entry go
-- red -- a previous wave shipped a token that was a substring of another
-- statement and stayed green when its own code was removed.
-- ── complete_sale ─────────────────────────────────────────────────────────
-- THE SIGNATURE CHANGES, so this one is DROP-then-CREATE rather than a plain
-- replace. `create or replace` with a different parameter count adds an
-- OVERLOAD instead of replacing, and the fourteen-argument call every existing
-- caller makes would then be ambiguous between the two and fail outright. The
-- drop names the old signature exactly; every migration that has added a
-- parameter to this function since 0005 does the same thing.
--
-- Dropping also takes the GRANTS with it, which is why the pair at the foot of
-- this file is not decoration here: without it PUBLIC -- which includes anon --
-- would hold the default EXECUTE grant Postgres puts on every new function, on
-- a SECURITY DEFINER function that runs as the owner.
drop function if exists public.complete_sale(uuid, jsonb, jsonb, text, text, text, text, integer, uuid, timestamptz, uuid, integer, uuid, boolean);
...
begin
  ...
  -- ── TAKING THE TAX OUT OF THE PRICE IS TAKING IT OUT OF THE TILL ───────
  --
  -- The flag moves no price, so it looks at first like it needs no permission.
  -- It moves what the shop KEEPS. The same two items at a 5% shop collect 2520
  -- and leave 2400 of revenue without it, and collect 2400 and leave 2286 with
  -- it: 114 cents of the shop's own takings, decided by a boolean in a JSON
  -- payload. That is the same money 20260929000050 gated the agreed price's
  -- undercut to protect, reached through a different field, and a new field is
  -- not a new justification.
  --
  -- ON THE EFFECT, NOT ON THE FIELD, which is the same shape as that gate. The
  -- permission is asked only where there is actually tax to extract: a shop
  -- with tax disabled, or charging zero, hands the caller the identical sale
  -- either way, and refusing it there would be a gate on the FLAG rather than
  -- on what the flag does. Baseline check 30 is all three halves of that.
  --
  -- BEFORE ANYTHING IS WRITTEN, for the reason the manual-discount check above
  -- gives: everything it reads is known here, so there is no sense doing the
  -- sale, the stock, the payments and the loyalty ledger first only to roll it
  -- all back.
  --
  -- READ THIS BEFORE WRITING TASK 4. Storefront fulfilment will set this flag,
  -- so at a tax-charging shop the member completing an order must hold
  -- `discounts.manual` -- the same requirement, and the same population, as the
  -- agreed-price undercut 20260929000050's header warns about: 20260826000100
  -- backfilled the permission onto every role that held pos.access, so only a
  -- shop that has deliberately taken it away from somebody feels this. Surface
  -- it as a permission problem the shop can act on rather than discovering it
  -- in a shop.
  --
  -- IT CANNOT SEE THE TOTAL, and does not try to. A sale whose total happens to
  -- come to zero extracts no tax and is still refused here; the alternative is
  -- to ask the permission after the cart has been priced and the stock moved,
  ...
  if v_prices_include_tax
     ...
     and not public.has_shop_permission(p_shop_id, 'discounts.manual') then
    raise exception 'not authorized to file a sale at prices that already include tax';
  end if;
