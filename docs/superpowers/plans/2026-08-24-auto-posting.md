...
| ~~Matching a bill to the delivery it pays for~~ | **No longer out — shipped in `20260908001900`.** `invoices.stock_receipt_id`, and `post_expense_to_ledger` keying on it INSTEAD OF on the category — which was a guess about something the database was never told, and the same shape as #74's false `invoice_id` premise. The link is asked **before** the category, so a bill for goods mis-tapped as `supplies` and linked now posts nothing where it used to post `Dr 6400 / Cr 2000` on top of the delivery's own credit and **double the payable** — that closes the OVER-STATED direction outright. The UNDER-STATED direction is closed **at the door, not in the ledger**: `guard_invoice_delivery_link` refuses an `inventory_purchase` bill that names no delivery (in a shop that has the `inventory` module and can therefore record one), refuses an uncosted delivery — it raised no payable to settle — refuses a delivery from another shop, and refuses any later change to the link. **The obvious branch was rejected and the reasoning is the point.** Posting `Dr 1200 / Cr 2000` for an unlinked goods bill cannot tell *the delivery was recorded and only the link is missing* (→ stock and payable both doubled) from *the goods were never recorded at all* (→ ledger value rises while quantity does not, **and** `opening_inventory_gap` shrinks by the phantom for ever, which is `20260908001800`'s negative asset re-entering from the other end). In the second case the shop's stock records are already wrong whatever the ledger does, so posting hides that rather than fixing it — `20260804000000`'s instinct exactly. **The link is IMMUTABLE**, and that is not conservatism: `expenses_post_to_ledger_on_edit` fires on a fixed column list and nothing on `expenses` moves when the *invoice's* link does, so the posting could not follow a link that moved and the live entry and the replay would disagree about one row. Correcting a mis-picked delivery is delete-and-re-enter, already complete since Task 7d. **Existing bills are untouched**: none carries a link, the historical arm posts exactly what it posted yesterday live and on replay, and nothing is re-posted — what changed is that the arm is now an *admitted gap* rather than a claim that a delivery recognised the cost. `unposted_ledger_source_rows` and `backfill_shop_ledger` step 1 grew the identical clause; `unposted_inventory_movement` and `opening_inventory_gap` read the view and inherited it. The Bills screen flags the offending rows per bill (badge + a line of meta, deliberately not a `Caveat` — seven amber blocks down a list is how the tone system dies) and the payable `Caveat` keeps `wrong` and its action, with one clause added saying it cannot recur. `docs/design/bill-delivery-link-mockup.html`, `verify-posting-bills.sql` checks 22–25 (24 is the door's six arms, including the cross-shop refusal and the no-`inventory`-module gate; 25 is `unbilled_stock_receipts` behaviourally — tenancy, the already-billed exclusion, the figures, the search and the `invoices.manage` gate), `verify-backfill.sql` check 22 (the same shop built twice — live and replayed — compared account by account), `invoice-delivery-link.test.tsx`, `accumulated-rpc-edits.test.ts`. |
| ~~Revaluing a product that is costed *after* it already had stock~~ | **No longer out — shipped in `20260908001800`, `receive_stock` copied forward from `20260908000400`.** `opening_inventory_gap` excludes uncosted stock, argued partly on the grounds that an uncosted product's whole life is invisible to `1200`. **That held only while it stayed uncosted.** `receive_stock` costs the *entire holding* at the delivery's price when the prior cost is null, so the opening quantity acquired a cost it never contributed: 50 uncosted units open at 0, a delivery of 10 @ 100 costs all 60 and debits `1200` by 1,000, selling all 60 credits `1200` by 6,000, and `1200` ended at **−5,000** — the same negative asset the opening balance exists to remove, and the opening marker meant a second replay could never correct it. **The exclusion is unchanged and still right** (there is no honest value for stock nobody has priced, and `price_cents` capitalises unearned profit). What was owed was the other end, and it is now posted: when a delivery moves `products.cost_cents` off null on a product that *already held stock*, `receive_stock` writes a **second entry** — `Dr 1200 Inventory / Cr 3000 Owner's Capital` for `v_prior_qty × v_new_cost`, dated `shop_local_date()`, sourced `'stock'` and carrying no location. **Two entries, not two extra lines**: a merged entry would stop the delivery's `1200` debit equalling what the supplier is owed, and deleting the delivery would reverse a revaluation that is *still true* (nothing un-costs the product). **`'stock'` and emphatically not `'opening'`** — `'opening'` would set `opening_inventory_gap`'s idempotency marker and suppress the shop's real opening balance. **Neither `backfill_shop_ledger` nor `unposted_ledger_sources` needed a change**, and that is a result rather than an omission: a revaluation is a posted entry with a line on `1200`, so it joins the gap's ledger term at once, and it has no source row, so it never joins the *about to be posted* term or the door. The two still cannot disagree about what a run will write. `verify-posting-inventory.sql` check 13 (a–e, plus 13c-ii's **free** delivery — a cost of *zero*, not an absent one, the one case where the revaluation's arithmetic comes out at 0 and `journal_lines`' `check (amount_cents <> 0)` would take the whole delivery down), `verify-backfill.sql` check 21b, `accumulated-rpc-edits.test.ts`. **`3000` is the best available account, not a description of every unit it values:** the header now names the second population it reaches — a delivery recorded through Restock with the cost field empty and costed later by a second delivery, whose units came from a supplier and may still be owed for. |
| A **linked bill whose amount differs from the delivery's costed value** | The delivery credited `2000` at cost; paying the bill debits it at the bill's figure, and carriage or a rounding leaves the difference standing in Accounts Payable. The form prefills the amount from the delivery and names the difference when it is edited, which is the whole defence — posting it honestly needs a goods-received-not-invoiced or price-variance account and that is phase 3. A **bounded** residue where the one it replaces was unbounded. |
| **One bill covering several deliveries**, or one delivery split across several bills | The column is one receipt per bill and the index is unique, so a bill covering three deliveries can name one and the other two stay unbilled with their payables standing. A join table is the real answer. The unique index at least stops two bills quietly settling the same delivery, which would drive `2000` negative by a whole delivery's value. |
| **A bill that arrives before the goods** | Refused, with the reason. There is no delivery to name yet, and a payable for inventory the shop does not hold is a prepayment rather than a stock purchase. Named because a refusal a user cannot explain is how a feature stops being used. |
| **A bill for a delivery that was received with no costs on it** | **Refused, and there is no in-app way to put the cost on afterwards** — `stock_receipts` has one policy (`read stock_receipts`), no update, no delete, no RPC and no UI. An uncosted delivery is an *ordinary* outcome (the restock sheet leaves the cost field empty on purpose and `receive_stock` accepts a null cost), so this is reachable by a shop that did nothing unusual. What the copy must never say — and said, until the residues review — is *receive it again with what it cost*: on 50 units already on the shelf that upserts to **100 units**, posts the delivery's `Dr 1200 / Cr 2000` **and** `20260908001800`'s revaluation `Dr 1200 / Cr 3000`, and nothing takes it back except a stock count booking the phantom as `Dr 5100 Inventory Shrinkage`. The three places that carried the sentence — `20260908001900`, `invoice-editor-modal.tsx` and the mockup — now name the two remedies that exist (a different delivery, or a bill that is not for goods saying so) and the prevention (enter the cost as you receive). `canSave` blocks it as well as warning. **The real fix is editable delivery costs, or a bill that can carry its cost onto the delivery it names. Phase 3.** |
| ~~The picker's cap~~ | **Closed in the residues review.** It was 25, newest first, with no search — and because no pre-existing bill carries a link, `unbilled_stock_receipts`' `not exists` makes *every delivery the shop has ever received* unbilled, so the list starts at the whole history. Receive through the week, enter the bills on Friday, and the older half is off the end while `needsDelivery` disables Save and the copy says "Pick the delivery above" for a delivery that is not above — leaving mis-classification (goods into `6400 Supplies`) as the only way out. Now 100, with a `p_search` that matches supplier, reference or the date as the picker prints it and runs **in the database**, because the delivery a shop cannot find is by definition past the end of the page. `verify-posting-bills.sql` check 25. |
| **A goods bill entered under another category AND not linked** | Still doubles the payable. The door cannot demand a link for `supplies` — a supplies bill usually is one — so all that stands is that the picker is offered on **every** category with the delivery's own reference printed in it. A mitigation, not a fix. |
...
*A bill could not say which delivery it paid for, so the trigger guessed by category and was wrong in both directions. **Shipped: `20260908001900`, `invoices.stock_receipt_id` and `guard_invoice_delivery_link()`.** See the row in *What is NOT in this plan*. **This was the last named residue of the phase**; what remains under that heading is either phase-3 work or a bounded consequence of this one, stated there.*
...
| `supabase/migrations/20260908001800_revalue_stock_costed_after_the_fact.sql` | Closes the revaluation residue. `receive_stock`, copied forward from `20260908000400` (**not** `20260907000000`, which would delete the posting side): a delivery that costs a product which already held stock posts a second entry, `Dr 1200 / Cr 3000 Owner's Capital` for the units already on the shelf. |
